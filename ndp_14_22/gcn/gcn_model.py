...
from gcn.gcn_layers import ResidualGatedGCNLayer, MLP
from utilsfile.model_utils import *
...
class ResidualGatedGCNModel(nn.Module):
    ...
    def __init__(self, config, dtypeFloat, dtypeLong):
        ...
        self.edges_values_embedding = nn.Linear(3, self.hidden_dim//2, bias=False)
...
#        self.mlp_edges = MLP(self.hidden_dim, self.voc_edges_out, self.mlp_layers)
    ...
    def forward(self, x_edges, x_edges_values, x_nodes, x_nodes_coord):
        ...
        e_vals = self.edges_values_embedding(x_edges_values)  # B x V x V x H
...
#        y_pred_nodes = self.mlp_nodes(x)  # B x V x voc_nodes_out
        y_pred_nodes = x.mean(1)  # B x hidden_dim    
