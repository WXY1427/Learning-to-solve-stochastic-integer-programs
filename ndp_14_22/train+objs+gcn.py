...
#         tracker_epoch = defaultdict(lambda: defaultdict(dict))
        ...
        ob_my_dataset = TensorDataset(tensor_x[0:args.num_labeled_ins].repeat(times,1,1),tensor_c[0:args.num_labeled_ins].repeat(times,1,1,1), tensor_obj.repeat(times,1))    
        ...
        for iteration, (x, y) in enumerate(data_loader):
            ...
            if args.conditional:
                ...
                recon_x, mean, log_var, z, target, _, _, _ = vae(x)
...
#                tracker_epoch[id]['label'] = yi.item()
            ...
            recon_x, mean, log_var, z, target, obj_pre, ind, _ = vae(x_ob.to(device), y_ob.to(device))    
        ...
        if epoch % args.save_net == 0 or epoch == args.epochs-1:
            # Specify a path
            PATH = "trained_nets/ob_saved_model_GCN_"+ str(epoch) + ".pt"
            # Save
