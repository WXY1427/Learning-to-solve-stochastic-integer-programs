...
#     torch.save(tensor_dist, 'tensor_dist_obj1020_46.pt')     
#     torch.save(tensor_obj, 'tensor_obj_obj1020_46.pt')  
#     torch.save(tensor_c_fn, 'tensor_c_fn_obj1020_46.pt')  
#     torch.save(tensor_c_cn, 'tensor_c_cn_obj1020_46.pt')      
#     torch.save(tensor_x, 'tensor_x_obj1020_46.pt')   

    tensor_dist = torch.load('tensor_dist_obj1020_46.pt')     
    tensor_obj = torch.load('tensor_obj_obj1020_46.pt')
    tensor_c_fn = torch.load('tensor_c_fn_obj1020_46.pt') 
    tensor_c_cn = torch.load('tensor_c_cn_obj1020_46.pt') 
    tensor_x = torch.load('tensor_x_obj1020_46.pt') 
...
#             recon_x.view(-1, 4), x.view(-1, 4), reduction='sum')
    ...
    for epoch in range(args.epochs):
        ...
        ob_my_dataset = TensorDataset(tensor_x[0:args.num_labeled_ins].repeat(times,1,1),tensor_c_cn[0:args.num_labeled_ins].repeat(times,1,1),tensor_c_fn[0:args.num_labeled_ins].repeat(times,1,1),tensor_dist[0:args.num_labeled_ins].repeat(times,1,1), tensor_obj.repeat(times,1))    
        ob_data_loader = DataLoader(
            dataset=ob_my_dataset, batch_size=args.batch_size, shuffle=True, drop_last=True, num_workers = 1).__iter__()           
        ...
        for iteration, (x, y_cn, y_fn, y_dist) in enumerate(data_loader):
            ...
            recon_x, mean, log_var, z, target, _, _, _ = vae(x, y_cn, y_fn, y_dist, epoch%n_scenarios)   
            
            loss = 0.01 * loss_fn(recon_x, target, mean, log_var) ###0.01


            # compute regrssion loss 
            x_ob, y_cn_ob, y_fn_ob, y_dist_ob, obj_tar = ob_data_loader.next()
            recon_x, mean, log_var, z, target, obj_pre, ind, _ = vae(x_ob.to(device), y_cn_ob.to(device), y_fn_ob.to(device), y_dist_ob.to(device))    
            loss += torch.nn.functional.mse_loss(obj_pre.view(-1, 1), obj_tar[:,ind].to(device).view(-1, 1))            

#             loss = 0.01 * loss_fn(recon_x, target, mean, log_var)+torch.nn.functional.mse_loss(obj_pre.view(-1, 1), obj_tar[:,ind].to(device).view(-1, 1))
        ...
        if epoch % args.save_net == 0 or epoch == args.epochs-1:
            # Specify a path
            PATH = "trained_nets/1ob_saved_model_GCN_dependence_"+ str(epoch) + ".pt"
            # Save
